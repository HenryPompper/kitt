...
def visualizer():
    ...
    while play:
        ...
        audio_animation(level)
        #speech_animation(level)
        ...
        if frameCounter > interfaceRate:
            ...
            if levelDisplayStr == '':
                levelDisplayStr = "0"
        titleTxt = font.render("K.I.T.T. v0.2", 1, configColor)
        visTxt = fontSmall.render("Visualizer Input: " + str(levelDisplayStr), 1, configColor)
        clockTxt = fontClock.render(datetime.now().strftime("%I:%M:%S"), 1, configColor)
        screen.blit(titleTxt, (10,10))
        screen.blit(visTxt, (20,36))
        screen.blit(clockTxt, (size[0] - 160,size[1] - 40))
        pygame.display.flip()
        clock.tick(60)
    pygame.quit()
def init():
    pygame.init()
    pygame.display.set_caption("K.I.T.T. | Computer Integrated Talking Terminal")
